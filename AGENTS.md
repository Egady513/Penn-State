...
---
## This repo: Penn State event apps

- Rally For Rescues pickleball registration, Drive Out Hunger golf outing
- The Greater Cincinnati Penn State Alumni Association is a registered
  **501(c)(3)**. Public-facing accuracy matters more than usual here.

**Never share a registration link until `penn-state-qa` has passed against the
live deployed site**, not a local build. That gate loads every page, checks for
500s and console errors, and verifies the registration to payment to
confirmation-email chain. Codex cannot run it. Hand off to Claude Code.

Stripe: test-mode and live-mode webhooks are separate endpoints with separate
signing secrets. Never reuse one event's secret for another.
